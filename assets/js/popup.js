...
class Popup {
    ...
    createPopUp(data) {
        const $currentPopup = this.$popup = this.$popup || $(data.tag || '<div/>');
    ...
    tryCreateLayer($body) {
        const $domLayer = $(document.querySelector('.popup-layer'));
        if ($domLayer.length > 0) {
    ...
    tryAddCloseButtonEvent($popup) {
        ...
        if ($domCloseButton.length === 0) { return; }
